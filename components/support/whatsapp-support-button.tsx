...
export function WhatsAppSupportButton({ href, label }: { href: string; label: string }) {
  return (
    ...
    >
      ...
      <span
        aria-hidden
        className="pointer-events-none absolute right-full top-1/2 mr-3 -translate-y-1/2 whitespace-nowrap rounded-[var(--radius-button)] bg-[var(--color-foreground)] px-2.5 py-1 text-xs font-medium text-[var(--color-background)] opacity-0 shadow-md transition-opacity duration-150 group-hover:opacity-100 group-focus-visible:opacity-100"
      >
        {label}
      </span>
    </a>
