...
import { createGrandTestAction, updateGrandTestAction, type GrandTestFormState } from "./actions";
...
export interface GrandTestInitial {
  examId: string;
  title: string;
...
export function GrandTestForm({
  ...
  exams: ExamTree[];
  grandTestId?: string;
  initial?: GrandTestInitial;
}) {
  const isEdit = Boolean(grandTestId);
  const action = isEdit ? updateGrandTestAction.bind(null, grandTestId!) : createGrandTestAction;
  const [state, formAction] = useActionState<GrandTestFormState, FormData>(action, {});
  const formRef = useRef<HTMLFormElement>(null);

  const [examId, setExamId] = useState(initial?.examId ?? exams[0]?.id ?? "");
  const [questionCount, setQuestionCount] = useState(initial?.questionCount ?? 100);
    ...
    <form ref={formRef} action={formAction} className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <Input id="title" name="title" required defaultValue={initial?.title} placeholder="Grand Test — Full Syllabus 1" />
        </div>
        <div className="flex flex-col gap-1.5">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <Input
            ...
            min={1}
            required
            defaultValue={initial?.durationMinutes ?? 180}
          />
        </div>
        <div className="flex flex-col gap-1.5">
      ...
      <div className="flex items-center gap-3">
        <SubmitButton label={isEdit ? "Save Changes" : "Create Grand Test"} pendingLabel={isEdit ? "Saving…" : "Creating…"} />
        {state.error ? (
        ...
        {state.success && !isEdit ? <p className="text-sm text-[var(--color-success)]">Grand test created as Draft.</p> : null}
