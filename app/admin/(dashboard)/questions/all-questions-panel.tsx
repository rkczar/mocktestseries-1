...
export async function AllQuestionsPanel({ examId, status }: { examId?: string; status?: string }) {
              ...
              <tbody>
                {questions.map((q) => (
                  <tr key={q.id} className="border-b border-[var(--color-border)] last:border-0 align-top">
                    ...
                    <td className="py-2.5 pr-4">
                      <Link href={`/admin/questions?tab=add&id=${q.id}`} className="text-[var(--color-primary)] hover:underline">
                        Edit
                      </Link>
                    </td>
                  </tr>
